...
class DetailImageView: UIView, UICollectionViewDataSource, UICollectionViewDelegate, SKPhotoBrowserDelegate, AVPlayerViewControllerDelegate, UICollectionViewDragDelegate, UIActivityItemSource {
    ...
    private func commonInit() {
        accessibilityIdentifier = "DetailImagView"
        ...
        layout.itemSize = CGSize(width: bounds.width, height: bounds.width)
        ...
        NSLayoutConstraint.activate([
            ...
            collectionView1.topAnchor.constraint(equalTo: topAnchor),
            collectionView1.bottomAnchor.constraint(equalTo: bottomAnchor),
        ])
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if images.isEmpty {
            ...
            cell.preferredWidth = bounds.width
            ...
            if let ur = URL(string: images[indexPath.item]) {
                ...
                if let layout = collectionView.collectionViewLayout as? ColumnFlowLayoutD {
                    layout.preferredWidth = bounds.width
                    layout.prepare()
            ...
            if isSensitive && GlobalStruct.blurSensitiveContent {
    ...
    func setupImages(url1: String, url2: String?, url3: String?, url4: String?, isVideo _: Bool? = false, videoUrl _: String? = "www.google.com", altText: [String] = [], fullImages: [Attachment] = [], isSensitive: Bool = false) {
    ...
    func setupPlayButton(_ videoUrl: String, isAudio: Bool = false) {
        ...
        if videoUrl != "" {
            ...
            if let cell = collectionView1.cellForItem(at: IndexPath(item: 0, section: 0)) as? CollectionImageCellD {
                playButton.frame = CGRect(x: cell.image.bounds.width / 2 - 35, y: cell.image.bounds.height / 2 - 35, width: 70, height: 70)
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if videoUrl != "" {
            ...
            if isVideo {
                ...
                var images = [SKPhoto]()
                if let cell = collectionView1.cellForItem(at: indexPath) as? CollectionImageCellD {
                    if let originImage = cell.image.image {
                        ...
                        let browser = SKPhotoBrowser(originImage: originImage, photos: images, animatedFromView: cell.image, imageText: postText, imageText2: 0, imageText3: 0, imageText4: "")
    ...
    func collectionView(_: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point _: CGPoint) -> UIContextMenuConfiguration? {
        if videoUrl != "" {
            return UIContextMenuConfiguration(identifier: indexPath as NSIndexPath, previewProvider: {
                ...
                self.makeContextMenuV(indexPath.row)
            })
        } else {
            return UIContextMenuConfiguration(identifier: indexPath as NSIndexPath, previewProvider: {
    ...
    func activityViewControllerLinkMetadata(_: UIActivityViewController) -> LPLinkMetadata? {
        var image1 = UIImage()
        if let cell = collectionView1.cellForItem(at: IndexPath(item: tmpIndex, section: 0)) as? CollectionImageCellD {
            image1 = cell.image.image ?? UIImage()
        }
        let image = image1
    ...
    func updateFromStat(_ stat: Status) {
        ...
        if z.first?.type == .video || z.first?.type == .gifv || z.first?.type == .audio {
            ...
            if z.first?.type == .audio {
                setupPlayButton(z.first?.url ?? "", isAudio: true)
            } else {
                setupPlayButton(z.first?.url ?? "")
            }
        } else {
        ...
        setupImages(url1: mediaItems ?? "", url2: mediaItems1, url3: mediaItems2, url4: mediaItems3, isVideo: isVideo, videoUrl: nil, altText: alt, fullImages: z, isSensitive: stat.reblog?.sensitive ?? stat.sensitive ?? false)
    }
}
