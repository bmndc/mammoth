...
class PostCell: UITableViewCell {
    ...
    func initialSetup() {
        ...
        p.addFillConstraints(with: contentView)
    }
...
extension PostCell {
    func configure(status: Status) {
        ...
        if traitCollection.userInterfaceStyle == .light {
            contentView.backgroundColor = .custom.backgroundTint.darker(by: 2)
        } else {
            contentView.backgroundColor = .custom.backgroundTint.lighter(by: 4)
        }
