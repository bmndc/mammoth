        ...
        if show != showingSpinner {
            ...
            if show {
                ...
                if activityIndicator == nil {
                    ...
                    superview?.insertSubview(activityIndicator, aboveSubview: self)
...
class ProfileCell: UITableViewCell, SKPhotoBrowserDelegate {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        headerImage.addTarget(self, action: #selector(headerTapped), for: .touchUpInside)
        ...
        contentView.insertSubview(lockedBackground, belowSubview: lockedBadge)
        ...
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[headerImage]-0-|", options: [], metrics: nil, views: viewsDict))

        #if targetEnvironment(macCatalyst)
            if GlobalStruct.singleColumn {
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[profileIcon(120)]", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[userName]-10-[lockedBackground(fontSize)]-(>=40)-|", options: [], metrics: metricsDict, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[userName]-5-[lockedBadge]-(>=40)-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[userTag]-(>=6)-[followsYou]-(40)-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[stackView]-40-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[stackView2]-40-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-40-[fieldsText]", options: [], metrics: nil, views: viewsDict))
            } else {
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[profileIcon(120)]", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[userName]-10-[lockedBackground(fontSize)]-(>=20)-|", options: [], metrics: metricsDict, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[userName]-5-[lockedBadge]-(>=20)-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[userTag]-(>=6)-[followsYou]-(20)-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[stackView]-20-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[stackView2]-20-|", options: [], metrics: nil, views: viewsDict))
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[fieldsText]", options: [], metrics: nil, views: viewsDict))
            }
        #elseif !targetEnvironment(macCatalyst)
            if UIDevice.current.userInterfaceIdiom == .pad, GlobalStruct.singleColumn, window?.traitCollection.horizontalSizeClass != .compact {
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-20-[profileIcon(120)]", options: [], metrics: nil, views: viewsDict))
        ...
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[headerImage(115)]", options: [], metrics: nil, views: viewsDict))
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-38-[profileIcon(120)]", options: [], metrics: nil, views: viewsDict))
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[userName]-(-1.4)-[followsYou]", options: [], metrics: nil, views: viewsDict))
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[lockedBackground(fontSize)]-1-[userTag]", options: [], metrics: metricsDict, views: viewsDict))
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[lockedBadge]-(-4)-[userTag]", options: [], metrics: nil, views: viewsDict))

        constraints0 = NSLayoutConstraint.constraints(withVisualFormat: "V:|-166-[userName]-(-8)-[userTag]-3-[stackView]-8-[stackView2]-8-|", options: [], metrics: nil, views: viewsDict)
        contentView.addConstraints(constraints0)
    }
    ...
    func setupFields(_ enabled: Bool = false) {
        ...
        if enabled {
            fieldsText.alpha = 1
            constraints0 = NSLayoutConstraint.constraints(withVisualFormat: "V:|-166-[userName]-(-8)-[userTag]-3-[stackView]-10-[stackView2]-10-[fieldsText]-20-|", options: [], metrics: nil, views: viewsDict)
            _ = constraints0.map { x in
                x.isActive = true
            }
        } else {
            fieldsText.alpha = 0
            constraints0 = NSLayoutConstraint.constraints(withVisualFormat: "V:|-166-[userName]-(-8)-[userTag]-3-[stackView]-8-[stackView2]-20-|", options: [], metrics: nil, views: viewsDict)
            _ = constraints0.map { x in
                x.isActive = true
            }
        }
    ...
    func setupFollowButton(followState: FollowButtonState) {
        ...
        if follow.isHidden {
            ...
            let viewsDict = [
            ...
            #if targetEnvironment(macCatalyst)
                if GlobalStruct.singleColumn {
                    contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-(>=10)-[follow]-20-|", options: [], metrics: nil, views: viewsDict))
                } else {
                    contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-(>=10)-[follow]-20-|", options: [], metrics: nil, views: viewsDict))
                }
            #elseif !targetEnvironment(macCatalyst)
                contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-(>=10)-[follow]-20-|", options: [], metrics: nil, views: viewsDict))
            #endif
            contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-124-[follow]", options: [], metrics: nil, views: viewsDict))
        }
    }
        ...
        if stat?.emojis.isEmpty ?? false {
            ...
            if let z = stat?.emojis {
                _ = z.map {
                    ...
                    textAttachment.kf.setImage(with: $0.url, attributedView: self.bioText, completionHandler: { _ in
