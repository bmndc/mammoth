...
class QuotePostMutedView: UIView {
    ...
    @IBOutlet var nameLabelLeftConstrait: NSLayoutConstraint!
    ...
    func updateFromStat(_ stat: Status?) {
        if stat == nil {
            ...
            if let constrait = nameLabelLeftConstrait {
                NSLayoutConstraint.deactivate([constrait])
                accountLabel.leadingAnchor.constraint(equalTo: quotesImageView.trailingAnchor, constant: 8).isActive = true
            }
