...
class TrendsTopCell: UITableViewCell, UICollectionViewDataSource, UICollectionViewDelegate, SKPhotoBrowserDelegate {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[collectionView1]-0-|", options: [], metrics: nil, views: viewsDict))
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[collectionView1(230)]-0-|", options: [], metrics: nil, views: viewsDict))
    }
    ...
    func setupDots() {
        ...
        let aa = CGFloat(6 * (allPosts.count - 1))
        let widthofAllTogether = CGFloat(6 * allPosts.count) + aa
        for (c, _) in allPosts.enumerated() {
            let startPos = CGFloat(width / 2) - CGFloat(widthofAllTogether / 2)
            let dot = UIView()
            dot.tag = c + 20
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let ur = allPosts[indexPath.item].image ?? URL(string: "www.google.com") {
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let x = allPosts[indexPath.item].url {
