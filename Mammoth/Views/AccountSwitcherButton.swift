...
class AccountSwitcherButton: UIButton {
    override init(frame: CGRect) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(updateImageAndMenu), name: didSwitchCurrentAccountNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(updateImageAndMenu), name: didUpdateAccountAvatar, object: nil)

        updateButtonImage()
        updateButtonMenu()
        showsMenuAsPrimaryAction = true

        layer.borderWidth = (UIDevice.current.userInterfaceIdiom == .phone) ? 3.0 : 0.5
        layer.borderColor = (UIDevice.current.userInterfaceIdiom == .phone)
        ...
        insertSubview(blurredBG, belowSubview: imageView!)
        blurredBG.pinEdges()

        contentEdgeInsets = (UIDevice.current.userInterfaceIdiom == .phone) ? .init(top: 3, left: 3, bottom: 3, right: 3) : .zero
    ...
    override func layoutSubviews() {
        ...
        imageView?.layer.cornerRadius = frame.height / 2.0 - layer.borderWidth
    }
...
extension AccountSwitcherButton {
