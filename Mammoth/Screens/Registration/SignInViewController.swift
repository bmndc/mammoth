...
class SignInViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        textField.frame = CGRect(x: view.safeAreaInsets.left + 13, y: (navigationController?.navigationBar.bounds.height ?? 0) + 50 + safe + 10, width: view.bounds.width - view.safeAreaInsets.left - view.safeAreaInsets.right - 40, height: 45)
        tableView.frame = CGRect(x: 0, y: (navigationController?.navigationBar.bounds.height ?? 0) + 100 + safe + 19.5, width: view.bounds.width, height: view.bounds.height - safe - 100 - 20 - (navigationController?.navigationBar.bounds.height ?? 0))

        topBorder.frame = .init(x: 0, y: textField.frame.origin.y + textField.frame.size.height + 14, width: view.frame.size.width, height: 0.5)
    ...
    override func viewDidLoad() {
        ...
        if fromPlus {
            title = NSLocalizedString("newAccount.title", comment: "Sign up screen to choose your instance")
        } else {
            if isFromSignIn {
                title = NSLocalizedString("signIn.title", comment: "Sign in screen to choose your instance")
            } else {
                title = NSLocalizedString("communities.title", comment: "Screen to choose an instance")
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(newInstanceLogged), name: NSNotification.Name(rawValue: "newInstanceLogged"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(loadSpecificInstance), name: NSNotification.Name(rawValue: "loadSpecificInstance"), object: nil)
        ...
        if SignInViewController.allInstances.isEmpty {
            SignInViewController.loadInstances(isFromSignIn: isFromSignIn)
        } else {
            loadSpecificInstance()
        }
    }
    ...
    func loadAccount(_ instance: String) {
        ...
        present(signingInAlertController(), animated: true, completion: nil)
                ...
                self.dismiss(animated: true) {
                    if application.value == nil {
                        DispatchQueue.main.async {
                            ...
                            let op1 = UIAlertAction(title: NSLocalizedString("generic.findOutMore", comment: ""), style: .default, handler: { _ in
    ...
    func setupUI() {
        ...
        categoryButton.frame = CGRect(x: (UIApplication.shared.windows.first?.safeAreaInsets.left ?? 0) + 13, y: (navigationController?.navigationBar.bounds.height ?? 0) + 10 + safe, width: view.bounds.width - 40, height: 50)
        ...
        topBorder.backgroundColor = UIColor.custom.outlines.cgColor
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if returnedText == "" || returnedText == " " || returnedText == "  " {} else {
            if isFromSignIn {
                loadAccount(returnedText)
            } else {
                loadOther(returnedText)
            }
        }
        return true
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 && textField.text?.isEmpty as? Bool == true {
            ...
            let profileIcon = defaultInstance[indexPath.row].thumbnail ?? ""
            if let ur = URL(string: profileIcon) {
                cell.profileIcon.sd_setImage(with: ur, for: .normal)
            }

            cell.titleLabel.text = defaultInstance[indexPath.row].name

            if let shortDescription = defaultInstance[indexPath.row].info?.shortDescription {
                cell.bio.text = "\(shortDescription)"
            } else {
                cell.bio.text = "\(defaultInstance[indexPath.row].info?.categories?.first ?? placeholder)"
            }

            let users = Int(defaultInstance[indexPath.row].users ?? "0")?.formatUsingAbbrevation() ?? "0"
            let attachment1 = NSTextAttachment()
            ...
            let lang = (defaultInstance[indexPath.row].info?.languages?.first ?? "EN").uppercased()
            ...
            if (defaultInstance[indexPath.row].info?.prohibitedContent ?? []).contains("nudity_nocw") || (defaultInstance[indexPath.row].info?.prohibitedContent ?? []).contains("pornography_nocw") {
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0, textField.text?.isEmpty as? Bool == true {
            if isFromSignIn {
                loadAccount(defaultInstance[indexPath.row].name)
            } else {
                loadOther(defaultInstance[indexPath.row].name)
            }
        } else {
            if isFromSignIn {
                loadAccount(SignInViewController.filteredInstances[indexPath.row].name)
            } else {
                loadOther(SignInViewController.filteredInstances[indexPath.row].name)
            }
        }
    }
