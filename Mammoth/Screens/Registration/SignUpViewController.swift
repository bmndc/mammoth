...
class SignUpViewController: UIViewController, UITextFieldDelegate {
    ...
    var canSignUp: Bool = false {
        didSet {
            signUpButton.isUserInteractionEnabled = canSignUp
        }
    }
    ...
    override func viewDidLoad() {
        ...
        signUpButton.setTitleColor(UIColor(named: "Feint Contrast"), for: .normal)
        signUpButton.backgroundColor = UIColor(named: "OVRLY Med Contrast")
        signUpButton.layer.cornerRadius = 8
        signUpButton.addTarget(self, action: #selector(login), for: .touchUpInside)
        signUpButton.isUserInteractionEnabled = false

        // Have background taps end editing
        let tap = UITapGestureRecognizer(target: self, action: #selector(viewBackgroundTapped))
    ...
    func registerAccount(_: InstanceData) {
        // test account stuff
        let request = Accounts.registerAccount(username: usernameText, email: emailText, password: passwordText, agreement: true, locale: "en")
    ...
    @objc func keyboardWillChange(notification: Notification) {
        if let keyboardFrame: NSValue = notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue {
            ...
            keyboardHeight = keyboardRectangle.height - (UIApplication.shared.windows.first?.safeAreaInsets.bottom ?? 0) - 4
            adjustScrollViewForView(nil)
        }
    }
    ...
    @objc func viewBackgroundTapped(_: Any) {
    ...
    @IBAction func didEndOnExit(_ textField: UITextField) {
        ...
        if textField == password {
            ...
            adjustScrollViewForView(username)
        }
        if textField == username {
            username.resignFirstResponder()
            adjustScrollViewForView(password)
        }
    }
    ...
    @IBAction func textFieldDidChange(_ textField: UITextField) {
        ...
        if textField == email {
            ...
            if isValidEmail(email.text ?? "") || (emailText == "") {
                validEmail = true
        ...
        if textField == password {
            ...
            let passwordIsValid = passwordText.count >= 8
        ...
        if allFieldsFilled, passwordLength, validEmail, validUsername {
            ...
            signUpButton.setTitleColor(.custom.feintContrast, for: .normal)
        }
    }
    ...
    @IBAction func textFieldDidBeginEditing(_ textField: UITextField) {
        adjustScrollViewForView(textField)
    }
