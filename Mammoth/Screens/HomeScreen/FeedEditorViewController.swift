...
extension FeedEditorViewController: UITableViewDelegate, UITableViewDataSource, UITableViewDragDelegate {
    ...
    func tableView(_: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let feedTypeItem = viewModel.getInfo(forIndexPath: indexPath),
           let cell = tableView.dequeueReusableCell(withIdentifier: FeedEditorCell.reuseIdentifier, for: indexPath) as? FeedEditorCell
        {
            cell.configure(feedTypeItem: feedTypeItem) { item, action in
                ...
                case .delete:
                    ...
                    case let .hashtag(tag):
                        ...
                        alert.addAction(UIAlertAction(title: NSLocalizedString("profile.unfollow", comment: ""), style: .destructive, handler: { _ in
                            cell.showLoader()
                    ...
                    case let .channel(channel):
                        ...
                        alert.view.tintColor = .custom.highContrast
                        alert.addAction(UIAlertAction(title: NSLocalizedString("generic.unsubscribe", comment: "Unsubscribe button label"), style: .destructive, handler: { _ in
                            cell.showLoader()
                    ...
                    case let .community(instanceName):
                        ...
                        alert.view.tintColor = .custom.highContrast
                        alert.addAction(UIAlertAction(title: NSLocalizedString("generic.unsubscribe", comment: ""), style: .destructive, handler: { _ in
                            cell.showLoader()
    ...
    func tableView(_: UITableView, targetIndexPathForMoveFromRowAt sourceIndexPath: IndexPath, toProposedIndexPath proposedDestinationIndexPath: IndexPath) -> IndexPath {
        if let feedTypeItem = viewModel.getInfo(forIndexPath: proposedDestinationIndexPath) {
    ...
    func tableView(_: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        viewModel.moveItem(fromIndexPath: sourceIndexPath, toIndexPath: destinationIndexPath)
    }
}

// MARK: Appearance changes

extension FeedEditorViewController {
