...
final class ProfileViewSettingsButton: UIButton {
    ...
    init(_ onButtonPress: @escaping UserCardButtonCallback) {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(updateContextMenu),
    ...
    private func setupUI() {
        ...
        icon = UIImageView(image: FontAwesome.image(fromChar: "\u{f0c9}", size: 18, weight: .bold).withRenderingMode(.alwaysTemplate))
    ...
    @objc func onFollowStatusUpdate() {
        if let account = user?.account {
            user = UserCardModel(account: account, premiumAccount: user?.tippableAccount)
        }
    }
...
extension ProfileViewSettingsButton {
    ...
    private func createContextMenuAction(_ title: String, _ buttonType: UserCardButtonType, isActive _: Bool, data: UserCardButtonCallbackData?) -> UIAction {
        ...
        let action = UIAction(title: title,
                              ...
                              identifier: nil, attributes: buttonType == .block ? .destructive : UIMenuElement.Attributes())
        { [weak self] _ in
