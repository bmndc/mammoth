...
private let indexOfProfileButton: Int = 4
...
class SidebarViewController: UIViewController, UICollectionViewDelegate, UIPencilInteractionDelegate, ASWebAuthenticationPresentationContextProviding, ShareableViewController {
    static let shared = SidebarViewController()
    ...
    var undoImCount = 0
    var fetchPadFirstTimeCalled: Date?
    var fetchPadTimer: Timer?
    ...
    override func viewDidLayoutSubviews() {
        ...
        undoBG.frame = CGRect(x: 15, y: UIScreen.main.bounds.height - view.safeAreaInsets.bottom - 66, width: 260 - 30, height: 50)
    ...
    @objc func fetchPad() {
        ...
        if fetchPadTimer != nil {
            fetchPadTimer!.invalidate()
            fetchPadTimer = nil
        }

        if fetchPadFirstTimeCalled == nil {
            fetchPadFirstTimeCalled = Date()
        }
        // Consider that the 'app just launched' if it's within the first 3 seconds
        let appJustLaunched = fetchPadFirstTimeCalled!.timeIntervalSinceNow > -3.0

        // If the app just launched, use the timer; otherwise, just make the call
        if appJustLaunched {
            fetchPadTimer = Timer(fire: Date(timeIntervalSinceNow: 0.1), interval: 0.0, repeats: false, block: { _ in
                self.fireFetchPadNotifications()
            })
            RunLoop.current.add(fetchPadTimer!, forMode: .common)
        } else {
            fireFetchPadNotifications()
        }
    }
    ...
    override func didMove(toParent parent: UIViewController?) {
        ...
        if let viewSuperview = view.superview {
            view.addFillConstraints(with: viewSuperview)
        } else {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(selectItem), name: NSNotification.Name(rawValue: "selectItem"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(reloadAllPadSidebar), name: NSNotification.Name(rawValue: "reloadAllPadSidebar"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(reloadDat), name: NSNotification.Name(rawValue: "reloadDat"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(sidebarHighlight), name: NSNotification.Name(rawValue: "sidebarHighlight"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(newPost), name: NSNotification.Name(rawValue: "newPost"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(newMessage), name: NSNotification.Name(rawValue: "newMessage"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(scrollTo), name: NSNotification.Name(rawValue: "scrollTo"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(settingsTap), name: NSNotification.Name(rawValue: "settingsTap"), object: nil)
    ...
    @objc func longPressedNB(sender _: UILongPressGestureRecognizer) {
    ...
    @objc func startTimer() {
        if AccountsManager.shared.allAccounts.isEmpty {} else {
            ...
            GlobalStruct.timer1.invalidate()
            GlobalStruct.timer1 = Timer(timeInterval: 14.0, target: self, selector: #selector(fireTimerAllFeeds), userInfo: [], repeats: true)
    ...
    func makeContextProfileMain() -> UIMenu {
        ...
        for index in 0 ..< AccountsManager.shared.allAccounts.count {
            ...
            let action1 = UIAction(title: "@\(AccountsManager.shared.allAccounts[index].fullAcct)", image: imV.image?.withRoundedCorners()?.resize(targetSize: CGSize(width: 20, height: 20)) ?? im, identifier: nil) { _ in
                // switch account
                DispatchQueue.main.async {
    ...
    @objc func reloadDat() {
        let selItem = collectionView.indexPathsForSelectedItems
        collectionView.reloadData()
        collectionView.selectItem(at: selItem?.first ?? IndexPath(item: 0, section: 0), animated: true, scrollPosition: .top)
    }
    ...
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        ...
        NSLayoutConstraint.activate([
            accountSwitcherButton.topAnchor.constraint(equalTo: view.topAnchor, constant: 25),
            accountSwitcherButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
    ...
    func createLayout(_: Bool? = false) -> UICollectionViewLayout {
    ...
    @objc func selectItem() {
        collectionView.selectItem(at: IndexPath(item: GlobalStruct.sidebarItem, section: 0), animated: true, scrollPosition: .top)
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        controlBar(didSelect: indexPath.item)
    }
}
...
class LCell: UICollectionViewListCell {
    ...
    override func updateConfiguration(using state: UICellConfigurationState) {
        ...
        guard var cConfig = contentConfiguration?.updated(for: state) as? UIListContentConfiguration else { return }
