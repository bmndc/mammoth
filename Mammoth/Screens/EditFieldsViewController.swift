...
class EditFieldsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UITextViewDelegate {
    ...
    @objc func keyboardWillChange(notification: Notification) {
        if let keyboardFrame: NSValue = notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue {
            ...
            tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: keyHeight, right: 0)
        }
    }
    ...
    @objc func addTap() {
        ...
        if let cell = cells[IndexPath(row: 0, section: 0)] as? AltTextMultiCell {
            label1 = cell.altText.text ?? ""
        }
        var content1: String? = nil
        if let cell = cells[IndexPath(row: 1, section: 0)] as? AltTextMultiCell {
            content1 = cell.altText.text ?? ""
        }
        var label2: String? = nil
        if let cell = cells[IndexPath(row: 0, section: 1)] as? AltTextMultiCell {
            label2 = cell.altText.text ?? ""
        }
        var content2: String? = nil
        if let cell = cells[IndexPath(row: 1, section: 1)] as? AltTextMultiCell {
            content2 = cell.altText.text ?? ""
        }
        var label3: String? = nil
        if let cell = cells[IndexPath(row: 0, section: 2)] as? AltTextMultiCell {
            label3 = cell.altText.text ?? ""
        }
        var content3: String? = nil
        if let cell = cells[IndexPath(row: 1, section: 2)] as? AltTextMultiCell {
            content3 = cell.altText.text ?? ""
        }
        var label4: String? = nil
        if let cell = cells[IndexPath(row: 0, section: 3)] as? AltTextMultiCell {
            label4 = cell.altText.text ?? ""
        }
        var content4: String? = nil
        if let cell = cells[IndexPath(row: 1, section: 3)] as? AltTextMultiCell {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = NSLocalizedString("profile.edit.infoAndLinks", comment: "")
