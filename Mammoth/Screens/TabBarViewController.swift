...
class TabBarViewController: AnimateTabController, UIGestureRecognizerDelegate, UIDropInteractionDelegate, ASWebAuthenticationPresentationContextProviding, ShareableViewController {
    ...
    func adjustIndicatorFrames() {
        ...
        if customTabsImagesUnselected2[GlobalStruct.tab1Index] == "tray.full" {
            offsetX1 = 32
        }
        if customTabsImagesUnselected2[GlobalStruct.tab2Index] == "tray.full" {
            offsetX2 = 32
        }
        if customTabsImagesUnselected2[GlobalStruct.tab3Index] == "tray.full" {
            offsetX3 = 32
        }
        if customTabsImagesUnselected2[GlobalStruct.tab4Index] == "tray.full" {
            offsetX4 = 32
        }
        let a1 = ((tabBar.bounds.width / 5) * 1)
        let a2 = ((tabBar.bounds.width / 5) * 2)
        let a3 = ((tabBar.bounds.width / 5) * 3)
        let a4 = ((tabBar.bounds.width / 5) * 4)
        let z = (tabBar.bounds.width / 10)
    ...
    @objc func goToActivityTab(notification: Notification) {
        selectedIndex = GlobalStruct.tab3Index

        if let activityVC = selectedViewController?.children.first as? ActivityViewController {
    ...
    @objc func goToMessagesTab(notification: Notification) {
        selectedIndex = GlobalStruct.tab4Index

        if let mentionsVC = selectedViewController?.children.first as? MentionsViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(gotoH), name: NSNotification.Name(rawValue: "gotoH"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(gotoC), name: NSNotification.Name(rawValue: "gotoC"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(gotoE), name: NSNotification.Name(rawValue: "gotoE"), object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(showIndActivity), name: NSNotification.Name(rawValue: "showIndActivity"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(hideIndActivity), name: NSNotification.Name(rawValue: "hideIndActivity"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(showIndActivity2), name: NSNotification.Name(rawValue: "showIndActivity2"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(hideIndActivity2), name: NSNotification.Name(rawValue: "hideIndActivity2"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(fetchDataFromSettings), name: NSNotification.Name(rawValue: "fetchDataFromSettings"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(undoTapped), name: NSNotification.Name(rawValue: "undoTapped"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(setFromSwitch), name: NSNotification.Name(rawValue: "setFromSwitch"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(fetchCurrentUserData), name: NSNotification.Name(rawValue: "fetchUserData"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(gotoN), name: NSNotification.Name(rawValue: "gotoN"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(gotoS), name: NSNotification.Name(rawValue: "gotoS"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(showNewContent1), name: NSNotification.Name(rawValue: "showNewContent1"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(hideNewContent1), name: NSNotification.Name(rawValue: "hideNewContent1"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(showNewContent2), name: NSNotification.Name(rawValue: "showNewContent2"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(hideNewContent2), name: NSNotification.Name(rawValue: "hideNewContent2"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(fetchPost0), name: NSNotification.Name(rawValue: "fetchPost"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(fetchUser0), name: NSNotification.Name(rawValue: "fetchUser"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(restoreFromDrafts2), name: NSNotification.Name(rawValue: "restoreFromDrafts2"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(showComposer), name: NSNotification.Name(rawValue: "showComposer"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(cleanupTabBar), name: NSNotification.Name(rawValue: "cleanupTabBar"), object: nil)
    ...
    @objc func fetchPost0() {
        fetchPost(GlobalStruct.schemeId)
    }

    func fetchPost(_: String) {}

    func fetchPost2(_: String) {}

    @objc func fetchUser0() {
        fetchUser(GlobalStruct.schemeProfileName)
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if ToastNotificationManager.shared == nil {
            ToastNotificationManager.shared = ToastNotificationManager(hostWindow: view.window!)
        }

        newPostButton.delegate = self
        newPostButton.installInView(view, additionalBottomOffset: tabBar.bounds.height)
    ...
    @objc func startTimer() {
        if AccountsManager.shared.allAccounts.count > 0 {
            ...
            GlobalStruct.timer1.invalidate()
            GlobalStruct.timer1 = Timer(timeInterval: 7.0, target: self, selector: #selector(fireTimerAllFeeds), userInfo: [], repeats: true)
            RunLoop.current.add(GlobalStruct.timer1, forMode: .common)
        }
    }

    @objc func fireTimerAllFeeds(timer _: Timer) {
    ...
    func setupNewContentIndicators() {
        newContent1.removeFromSuperview()
        newContent1.frame = CGRect(x: tabBar.bounds.width / 10 - 4, y: tabBar.bounds.height - 40, width: 8, height: 8)
        newContent1.backgroundColor = .custom.baseTint
        newContent1.layer.cornerRadius = 4
        newContent1.alpha = 0
        tabBar.addSubview(newContent1)

        newContent2.removeFromSuperview()
        newContent2.frame = CGRect(x: ((tabBar.bounds.width / 10) * 2) - 4, y: tabBar.bounds.height - 40, width: 8, height: 8)
        newContent2.backgroundColor = .custom.baseTint
        newContent2.layer.cornerRadius = 4
        newContent2.alpha = 0
        view.addSubview(newContent2)
    }

    private func fillPath(for percent: Double) -> CGPath {
        let height = view.bounds.height * CGFloat(percent)
        let rect = CGRect(x: 0, y: height, width: view.bounds.width, height: view.bounds.height - height)
    ...
    @objc func fireTimer(timer _: Timer) {
        var count = (Int(counter.titleLabel?.text ?? "1") ?? 1) - 1
        if count < 0 {
            count = 0
        }
        counter.setTitle("\(count)", for: .normal)
    }
    ...
    @objc func hideIndActivity() {
        hideActivityUnreadIndicator()
    }

    @objc func showIndActivity() {
        showActivityUnreadIndicator()
    }

    @objc func hideIndActivity2() {
        hideMessagesUnreadIndicator()
    }

    @objc func showIndActivity2() {
        showMessagesUnreadIndicator()
    }
...
//        self.updateTabTitles()
        ...
        tabBar.items![3].imageInsets = UIEdgeInsets(top: 6, left: 0, bottom: -6, right: 0)
        tabBar.items![4].imageInsets = UIEdgeInsets(top: 6, left: 0, bottom: -6, right: 0)
    }
...
class AnimateTabController: AnimatedTabBarController, Jumpable {
    ...
    override func barItemTap(_ sender: Any) {
        ...
        if let itemIndex = animatedTabBar.indexOfTabBarItem(animatedTabBarItem) {
            if let navController = viewControllers?[itemIndex] as? UINavigationController, itemIndex == previousTappedIndex {
                navController.popToRootViewController(animated: true)
            }

            if itemIndex == previousTappedIndex {
                let tabBarItem = animatedTabBar.tabBarItems[itemIndex]
            ...
            controlBar(didSelect: itemIndex)
        }
    }
}
...
extension AnimateTabController {
    ...
    func barSingleTap(didSelect _: Int) {}
}
...
extension AnimateTabController {
    func storeSelectedIndex() {
        UserDefaults.standard.set(selectedIndex, forKey: "AnimateTabController selectedIndex")
    }

    func restoreSelectedIndex() {
        selectedIndex = UserDefaults.standard.value(forKey: "AnimateTabController selectedIndex") as? Int ?? 0
    }
}
...
extension TabBarViewController: AppStateRestoration {
    func storeUserActivity(in activity: NSUserActivity) {
        ...
        activity.userInfo?["ViewController.selectedIndex"] = selectedIndex
