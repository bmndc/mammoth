...
enum PostActions {
    static func onActionPress(target: UIViewController, type: PostCardButtonType, isActive: Bool, postCard: PostCardModel, data: PostCardButtonCallbackData?) {
        ...
        case .link:
            ...
            case let .mention((mention, status)):
...
extension PostActions {
    ...
    static func onDeletePost(target: UIViewController, postCard: PostCardModel, withFetchPolicy fetchPolicy: StatusService.FetchPolicy = .retryLocally) {
        ...
        alert.addAction(UIAlertAction(title: NSLocalizedString("generic.delete", comment: ""), style: .destructive, handler: { _ in
            Task {
                do {
                    _ = try await StatusService.delete(status: status, withPolicy: fetchPolicy)
    ...
    static func onUnpinPost(target _: UIViewController, postCard: PostCardModel, withFetchPolicy fetchPolicy: StatusService.FetchPolicy = .retryLocally) {
        guard case let .mastodon(status) = postCard.data else { return }
    ...
    static func onShare(target: UIViewController, postCard: PostCardModel) {
        ...
        onShare(target: target, url: text)
    }
    ...
    static func onCopy(target _: UIViewController, url: URL) {
    ...
    static func onTranslate(target _: UIViewController, postCard: PostCardModel) {
        guard case let .mastodon(status) = postCard.data else { return }
    ...
    static func onProfilePress(target: UIViewController, postCard: PostCardModel) {
        if let user = postCard.user {
            onProfilePress(target: target, user: user)
        }
    }
    ...
    static func openLinksB(url _: URL, url2: String, id: String) {
    ...
    static func report(postCard: PostCardModel, withFetchPolicy _: StatusService.FetchPolicy = .retryLocally) {
        guard let accountID = postCard.account?.id, case let .mastodon(status) = postCard.data else { return }
        ...
        alert.addAction(UIAlertAction(title: NSLocalizedString("post.report", comment: ""), style: .destructive, handler: { _ in
            Task {
                do {
