...
extension Account: Equatable {
    static func addAccountToList(account: Account) -> Bool {
        guard let accountsData = UserDefaults.standard.object(forKey: "allAccounts") as? Data, var accounts = try? PropertyListDecoder().decode([Account].self, from: accountsData) else {
    ...
    static func getAccounts() -> [Account] {
        guard let accountsData = UserDefaults.standard.object(forKey: "allAccounts") as? Data, let accounts = try? PropertyListDecoder().decode([Account].self, from: accountsData) else {
            return [Account]()
        }
        return accounts
    }
