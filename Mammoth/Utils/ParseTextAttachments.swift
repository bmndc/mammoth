...
public func formatRichText(string: NSAttributedString, label: UILabel, emojis: [Emoji]?) -> NSAttributedString {
    ...
    if let emojis = emojis {
        for emoji in emojis {
            ...
            textAttachment.kf.setImage(with: emoji.url, attributedView: label)
            ...
            while str.mutableString.contains(":\(emoji.shortcode):") {
                let range: NSRange = (str.mutableString as NSString).range(of: ":\(emoji.shortcode):")
