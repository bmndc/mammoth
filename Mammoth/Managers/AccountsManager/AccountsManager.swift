...
class AccountsManager {
    ...
    @MainActor func switchToAccount(_ acctData: (any AcctDataType)?, forceUIRefresh: Bool = true) {
    ...
    @MainActor func syncIdentityData() {
        if let identity = sanitizedCurrentIdentityData {
    ...
    func updateAccount(_ acctData: any AcctDataType) {
    ...
    func addExistingMastodonAccount(instanceData: InstanceData, client: Client, completion: @escaping (_ error: Error?, _ acctData: (any AcctDataType)?) -> Void) {
    ...
    func addNewMastodonAccount(instanceData: InstanceData, client: Client, completion: @escaping (_ error: Error?) -> Void) {
    ...
    // Signs in to an existing account, and adds it to the list of accounts
    func addExistingBlueskyAccount(authResponse: BlueskyAPI.AuthResponse) async throws {
        log.debug("account: adding existing account")
    ...
    func addNewBlueskyAccount(instanceData _: InstanceData, client _: Client, completion _: @escaping (_ error: Error?) -> Void) {
        // Bluesky HERE
    ...
    @MainActor func logoutAndDeleteAccount(_ acctData: any AcctDataType) {
        ...
        // • remove from data structures
        let isCurrentAccount = currentAccount?.isEqualTo(other: acctData) ?? false
        if isCurrentAccount {
            currentAccount = nil
        }
        allAccounts = allAccounts.filter { acct in
            !acct.isEqualTo(other: acctData)
        }
        ...
        switchToAccount(allAccounts.first)
    ...
    @MainActor private func restoreAccountsFromDisk() async throws {
        ...
        do {
            ...
            for networkAcctData in codedArray {
            ...
            for account in AccountsManager.shared.allAccounts {
                if let mastodonAcct = account as? MastodonAcctData {
                    if mastodonAcct.wentThroughOnboarding {
                        didShowOnboardingForAccount(mastodonAcct)
                    }
                }
            }
...
extension AccountsManager {
    var currentAccountClient: Client {
        if let currentAccount = currentAccount as? MastodonAcctData {
    ...
    var sanitizedCurrentIdentityData: IdentityData? {
        if let mastodonAccount = (currentAccount as? MastodonAcctData) {
            return IdentityData(from: mastodonAccount, allAccounts: allAccounts)
        }
...
extension AccountsManager {
    func updateCurrentAccountFromNetwork() {
    ...
    func updateCurrentAccountForYouFromNetwork() {
        if let account = currentAccount,
           let acctHandler = acctHandlerForAcctData(account)
        {
            Task {
    ...
    func updateCurrentAccountForYou(_ forYouAccount: ForYouAccount, writeToServer: Bool = true) {
        if let account = currentAccount,
           let acctHandler = acctHandlerForAcctData(account)
        {
            Task {
    ...
    func updateCurrentAccountForYou(_ forYouInfo: ForYouType, writeToServer: Bool = true) {
        if let account = currentAccount,
           let acctHandler = acctHandlerForAcctData(account)
        {
            Task {
    ...
    func updateCurrentAccountDisplayName(_ displayName: String) {
        if let account = currentAccount,
           let acctHandler = acctHandlerForAcctData(account)
        {
            Task {
    ...
    func updateCurrentAccountAvatar(_ avatar: UIImage, silently: Bool = false) {
        if let account = currentAccount,
           let acctHandler = acctHandlerForAcctData(account)
        {
            Task {
    ...
    func updateCurrentAccountHeader(_ header: UIImage) {
        if let account = currentAccount,
           let acctHandler = acctHandlerForAcctData(account)
        {
            Task {
...
// Onboarding
extension AccountsManager {
    func shouldShowOnboardingForCurrentAccount() -> Bool {
        var shouldShowOnboarding = true
        if let mastodonAcctData = currentAccount as? MastodonAcctData {
    ...
    func didShowOnboardingForCurrentAccount() {
        if let updatedAcctData = currentAccount as? MastodonAcctData {
            didShowOnboardingForAccount(updatedAcctData)
        }
    }

    func didShowOnboardingForAccount(_ mastodonAcctData: MastodonAcctData) {
