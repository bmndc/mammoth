...
final class NetworkMonitor {
    ...
    //
    var isNearRateLimit = false
    private var rateLimitCheckCounter = 0
    private var lastNearAlertDate: Date?
    ...
    func logNetworkCall(response: URLResponse?, isMothClient: Bool) {
    ...
    private func postRateLimitedAlert(withEmail: Bool = false) {
        ...
        if withEmail {
            ...
            DispatchQueue.main.async {
                ...
                alert.addAction(UIAlertAction(title: NSLocalizedString("generic.ok", comment: ""), style: .cancel, handler: { _ in
                }))
                if let presentingVC = getTopMostViewController() {
                    presentingVC.present(alert, animated: true, completion: nil)
                }
