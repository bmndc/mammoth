...
enum StatusService {
    ...
    static func like(postCard: PostCardModel, withPolicy fetchPolicy: FetchPolicy = FetchPolicy.regular) async throws -> PostCardModel? {
        ...
        case let .mastodon(status):
            ...
            case .retryLocally:
                return try await runTaskWithLocalRetry(forStatus: status) { id, _ in
                    let request = Statuses.favourite(id: id)
                    let result = try await ClientService.runRequest(request: request)
                    return PostCardModel(status: result)
                }
            case .onlyLocal:
                return try await runTaskLocally(forStatus: status) { id, _ in
                    let request = Statuses.favourite(id: id)
                    let result = try await ClientService.runRequest(request: request)
                    return PostCardModel(status: result)
                }
    ...
    @discardableResult
    static func report(accountID _: String, status: Status, withPolicy fetchPolicy: FetchPolicy = FetchPolicy.regular) async throws -> Report? {
        switch fetchPolicy {
        case .regular:
            if let id = status.reblog?.id ?? status.id {
        ...
        case .retryLocally:
            return try await runTaskWithLocalRetry(forStatus: status) { id, accountId in
                let request = Reports.report(accountID: accountId ?? "", statusIDs: [id], reason: "")
                let result = try await ClientService.runRequest(request: request)
                return result
            }
        case .onlyLocal:
            return try await runTaskLocally(forStatus: status) { id, accountId in
                let request = Reports.report(accountID: accountId ?? "", statusIDs: [id], reason: "")
                let result = try await ClientService.runRequest(request: request)
                return result
            }
            ...
            do {
                ...
                case let .mastodonError(message):
                    if message == "Record not found" {
                        return try await runTaskLocally(forStatus: status, task: task)
                    }
                default: break
                }
