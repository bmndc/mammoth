...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler _: @escaping () -> Void) {
        ...
        if let id = info["id"] as? Int64 {
            ...
            checkNotificationTypeForID(GlobalStruct.curIDNoti, alsoGoToTab: true)
        }

        // Open URL in customer.io push notifications
        if let urlStr = (info as? [String: Any])?[keyPath: "CIO.push.link"] as? String, let url = URL(string: urlStr) {
    ...
    func application(_: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if let userInfo = launchOptions?[UIApplication.LaunchOptionsKey.remoteNotification] {
            if let urlStr = (userInfo as? [String: Any])?[keyPath: "CIO.push.link"] as? String, let url = URL(string: urlStr) {
                let prevValue = GlobalStruct.openLinksInBrowser
                GlobalStruct.openLinksInBrowser = false
                PostActions.openLink(url)
                GlobalStruct.openLinksInBrowser = prevValue
    ...
    func applicationWillTerminate(_: UIApplication) {
        ...
        override func buildMenu(with builder: UIMenuBuilder) {
            ...
            // multi column switch
            let commandMC = UIKeyCommand(input: "]", modifierFlags: [.command], action: #selector(toggleMultiColumn))
            ...
            // quit
            let commandQ1 = UIKeyCommand(input: "q", modifierFlags: [.command], action: #selector(quitA))
        ...
        @objc func quitA() {
            _ = UIApplication.shared.windows.map { x in
                if let sess = x.windowScene?.session {
                    UIApplication.shared.requestSceneSessionDestruction(sess, options: nil) { _ in
                        log.error("err")
                    }
                }
            }
