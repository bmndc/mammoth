...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func stateRestorationActivity(for _: UIScene) -> NSUserActivity? {
        ...
        storeUserActivity(in: activity)
        return activity
    }

    private func storeUserActivity(in activity: NSUserActivity) {
        log.debug("SceneDelegate:" + #function)
        if let rootViewController = window?.rootViewController as? AppStateRestoration {
    ...
    func windowScene(_ windowScene: UIWindowScene, didUpdate _: UICoordinateSpace, interfaceOrientation _: UIInterfaceOrientation, traitCollection _: UITraitCollection) {
        #if !targetEnvironment(macCatalyst)
            if inBG == false {
                if UIApplication.shared.isRunningInFullScreen() == GlobalStruct.fullScreen {} else {
                    ...
                    if UIDevice.current.userInterfaceIdiom == .pad {
                        ...
                        setupWindows(forScene: windowScene)
                    }
                }
            }
        #endif
    ...
    func showAppLockedView() {
        ...
        if blurredEffectView == nil {
            blurredEffectView = UIVisualEffectView(effect: UIBlurEffect(style: .prominent))
            window?.addSubview(blurredEffectView!)
            blurredEffectView!.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        }
        blurredEffectView!.frame = UIApplication.shared.windows.last?.bounds ?? UIWindow().bounds
        window?.bringSubviewToFront(blurredEffectView!)
        blurredEffectView!.layer.zPosition = CGFloat(Float.greatestFiniteMagnitude)
    }
